...
class OrganismFactory:
    def __init__(self, confOrg, confOrgFac, confCon, confPssm):
        ...
        self.PWM_PROBABILITY_STEP = confOrgFac[
    ...
    def getOrganism(self):
        ...
        if random.random() < self.INITIAL_CONNECTOR_PROBABILITY:
            rootNode = self.createConnection(
                self.INITIAL_CONNECTOR_PROBABILITY * self.REDUCER_PROBABILITY_FACTOR
            )
        else:
    ...
    def createConnection(self, connectionProbability):
        ...
        if random.random() < connectionProbability:
            node1 = self.createConnection(
                connectionProbability * self.REDUCER_PROBABILITY_FACTOR
            )
        else:
    ...
    def getPwmColumn(self):
        ...
        while (
            leftProbability > minProbability and len(probabilities) < maxProbability - 1
        ):
    ...
    def importConnector(self, connector):
        newConnector = ConnectorObject(
            connector["mu"], connector["sigma"], self.confCon
        )
